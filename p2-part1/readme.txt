...
README File
  ...
  Instructions to Run:
    The program can be compiled via g++, and accepts an input file via cin. The simplest way to do this is via bash redirection. g++ will create an executable in the same folder, to run please use file redirection in conjunction with the executable.
  ...
  Input/Output:
	...
	Please use file redirection for simplicity when running our program, see the 'Instructions to Run' section of this document for more details.
  ...
  Justification of Abstractions, Data Structures, Algorithms, and Design Decisions:
    ...
    With the first, and follow sets generated we can then generate the parse table which is stored as a two dimensional vector of ints.
